...
public final class EnrichCacheStore: ObservableObject {
    ...
    public func saveEdit(key: String, lyrics: String, tr: String, roma: String, yrc: String? = nil,
                         ...
                         decision: [String: Any]? = nil, coverURL: URL? = nil) async -> Bool {
        var entry = raw[key] ?? [:]
        ...
        if let coverURL {
            if coverURL.absoluteString.isEmpty {
                entry.removeValue(forKey: "cover_url")
            } else {
                entry["cover_url"] = coverURL.absoluteString
            }
        }
