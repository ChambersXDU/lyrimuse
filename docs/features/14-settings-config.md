...
- 「歌词来源十源勾选」与解析器注册表保持一致；这里的十源只表示可勾选的歌词来源，不改变失败重试和挑选规则。
...
8. 设置页每行一个设置是通用版式；不再添加副标题、行首帮助图标或说明气泡，必要的运行状态直接用控件状态和错误反馈表达。
...
26. **「歌词显示」页把撤掉的页内大标题加回来，代价由窗口高度吃下（2026-09-13，用户拍板）**：
    ...
    - **副标题与说明统一移除（2026-09-20，用户拍板）**：设置页不再渲染页头副标题、行说明、帮助气泡和说明块，只保留标题、控件与必要状态。
