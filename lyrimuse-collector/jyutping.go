...
// "雅 ngaa5"),这里只取每个字的一个:优先取**没有权重标注**的那条(测试多个已知
// 多音字验证这是主读音,如"雅→ngaa5"/"中→zung1"/"和→wo4"/"我→ngo5"/"唔→m4"/
...
// 处理"离"这个字为什么读错发现的:简体字里有 71 个字,本身**同时**是
...
//
// 排查验证这不是"离"一个字的偶发情况:系统性比对字表和 OpenCC 简→繁映射,发现有
...
// 完整谚语(如"笑左，笑埋右"、"不自由，毋寧死",测试 103077 条 2 字以上词条里有 696 条
...
// 这个函数自己的契约"查不到读音的字符原样保留"(review 验证:
...
func toJyutpingLine(text string) string {
	...
	// 最后写出去的那一段是不是查出来的读音。
	//
	// ⚠️ :这里原来只用一个 needSpace,并在写出"查不到读音的字符"之后按
	...
	//     "babyngo5 oi3 nei5"、"OKlaa1"、"Do re midong1"(用户截图验证,
