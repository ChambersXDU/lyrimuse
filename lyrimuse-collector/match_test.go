...
// 固定样本回归测试——覆盖 验证的网易云"纯音乐曲目返回一份看着正常的
...
// 排查验证的真实 bug 的回归测试:同一首歌的不同录音(demo/original
...
func TestTitleVersionTags(t *testing.T) {
	cases := []struct {
		...
		// 中文版本限定词(真实bug案例:「蜗牛 (伴奏)」查 lyricfind,词表原来
		// 只认拉丁词,伴奏版被当成正常版收了)。
		...
		{"不插电的夏天", nil},
		// 裸 "edit"(PRINCE《Diamonds and Pearls (2023 Remaster)》配了《… (Edit)》的词,
...
func TestDefaultVersionTagSemantics(t *testing.T) {
	...
	// (同专辑、自报 180s):限定词对不上,但两道硬证据验证同一次录音 → 豁免。
...
func TestVersionTagsMismatch(t *testing.T) {
	cases := []struct {
		...
		// 复现:本地「蜗牛 (伴奏)」,lyricfind 召回的是正常演唱版
...
// 收紧的代价测试为零:250 首全量候选数据里,靠子串才被接受的候选一条都没有。
...
// 「搜索词也要去一次括号」——处理场景:各源拿去搜的一直是带
// "(Remastered 2014)" 的原样标题。当时只有 netease 在搜索词那一层调了 stripParens。
//
// 逐源测试(见 searchTitleVariants 注释)验证这不是洁癖问题:QQ 的 smartbox 只要 key 里
...
func TestSearchTitleVariants(t *testing.T) {
	cases := []struct {
		...
		// 时间轴对不上。测试:提前之后酷狗这条从 806 分掉到 207。
		{"版本限定词:原样优先",
...
// 「两个源一起抓错版本、互相印证」——匹配到的真实未命中。
//
// Valentina (feat. Rick Ross) [Bonus] 是 237s,QQ 和酷狗都匹配到了普通版(都停在 143s),
// 于是互相印证、各拿 +100 豁免掉时长惩罚;真正匹配到 bonus 版、末句 226s 的 LRCLIB 反倒
// 分数低一截,冠军判给了 QQ。
//
...
func TestTitleMatchTierPoints(t *testing.T) {
	cases := []struct {
		...
		{"候选没报标题", "", "Song", 0},
		// :中文限定词在 segmentVersionTags 里没有空格分词,"伴奏版"整段
...
// 处理「搜索候选歌词」把方大同《南音》的正确候选判成"语言跟这首歌对不上"
...
func TestIsProbablyWrongLanguageLyrics(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			// knownArtistAlias 那道豁免本身,不是"候选源给没给中文名"那道。			// 手工表缩到只剩两条通用机制(MusicBrainz+QQ)都覆盖不了的真实残留案例,
...
// :那英《微笑着离去》本地标签罗马化成 artist="Na Ying" title=
...
// TestAlbumTokensLatinCJKBoundary 固定 v9 的拉丁↔CJK 交界分词(周杰伦《The One》案:
...
// TestVersionTagsMismatchAlbumCJKLiveMarker 固定 v9 的 recordingVersionTags:
