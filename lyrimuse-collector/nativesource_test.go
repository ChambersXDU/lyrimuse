...
func TestNativeSourceBonus(t *testing.T) {
	...
	// 质量项的量级，就会重演 那次"按来源加分"的未命中（0 次变对、6 次变错）。
...
//
// (这里原来是 TestResolveNativeLyricSources,钉的是 多选那次的行为:
//
...
func TestSetNativeLyricSourcesForPlayer(t *testing.T) {
	...
	// ⚠️ 处理那个 bug 的形状:设置里六个播放器全勾,但实际在放 Apple Music。
...
// 案例：网易云《梦想家》Tango 2:44，Spotify 版 ~4:06，预取按 164s 选了短版歌词。
//
// 签名 改:mismatch 由调用方(trackEnrichment)用 durationMismatch 算好、
