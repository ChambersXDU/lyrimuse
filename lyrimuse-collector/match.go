...
// 不该被当成异常拦下来)。
//
// ⚠️ 两道豁免都是 补的,起因是同一个真实案例但对症的信号不一样:
// 处理「搜索候选歌词」把方大同《南音》的正确候选判成"语言跟这首歌对不上"——Apple
...
// genericHanCreditLineRe matches structural Chinese credit line prefixes (1-8 Han characters followed by a colon)
// to identify and filter production credits that are formatted as timed lyric entries.
// Distinguishes production staff entries ("Role: Name") from sung lyric lines.
...
func isCreditOnlyLRC(lrc string) bool {
	...
	// Singer speaker labels ("男：", "女：") are excluded from credit line filtering (lyricspeaker.go)
	// to prevent duet lyric lines with character prefixes from being discarded as credits.
...
// lastLRCTimestampSecs extracts the timestamp of the last actual lyric line in seconds.
// Skips empty timestamp placeholders and trailing credit lines (isCreditLineWithSpeakers)
// that lack sung lyrical content, ensuring accurate duration calculations.
...
type lyricCandidate struct {
	...
	// hasUsableTranslation/hasUsableRomanization: indicates whether the candidate carries
	// usable value-add translations or romanized text matching the configured target language.
	// Evaluated via usableValueAdd during candidate construction to prioritize enriched lyrics.
	hasUsableTranslation  bool
	hasUsableRomanization bool
	// sourceReportedDurationSecs: duration declared by the source in seconds (0 if unspecified).
	// Used in scoring to penalize duration mismatches against track playback duration (sourceDurationMismatchPenalty).
	sourceReportedDurationSecs float64
	// title/artist/album/cover: candidate metadata returned by the source for search dialog display.
	// Title participates in scoring to verify version qualifier consistency (versionTagsMismatch).
	title, artist, album, cover string
	// language: song language reported by candidate sources (e.g. Cantonese "yue" or Mandarin "cmn").
	// Used for Jyutping generation and language version verdicts (applyLanguageVersionVerdicts).
	language string
	...
	// plainTextOnly: true when lyrics contain non-timestamped plain text.
	// Bypasses isTimedLRC rejection and assigns score -1 so automated paths ignore it,
	// while allowing manual adoption in the search dialog.
	plainTextOnly bool
}
...
// corroboratedEndings returns the set of sources whose end timestamps are corroborated by at least one other source.
// Mutual timestamp agreement across independent providers confirms alignment and avoids penalizing tracks
// with extended instrumental outros. If any candidate in the batch matches the full duration within tolerance,
// exemptions are withheld to prevent shared false positives across variants.
...
// 处理 PRINCE《319》(The VERSACE Experience 那张里 88 秒的 X-cerpt 节选版)"搜不到"——四个源里
...
// 验证(联合 61 翻盘/5 纠错/0 回归/6 首手选金标签无损,评测器见 simeval_test.go):
...
// (0.83%,且两个都是真实版本误配,titleMatch 这条判据 0 误报)——处理方大同《公园》
...
// 用罗马化艺名+本地标签也是罗马化写法"这一类被误杀——处理方大同《南音》(本地标签
...
// 处理陈奕迅《Shall We Dance (Live)》/《活着多好 (Live)》案:本地专辑《The Easy Ride
...
// 处理"QQ 搜出来的是录音室版"——检索层的修复在 qq.go 的专辑维度路线,不需要 bump;
// 这两处是它牵出的打分层配套):
...
//	回放里匹配到并已修掉的两类边界:专辑名括号里的描述文案不算标记(韦礼安《女孩》案,
...
//
// ⚠️ **这次提版本号本身就是修复的一部分,不是走个形式**:被写坏的条目(测试用户库里
...
// 共识 +250、行数 -59、译文 +50(目标语言中文时);受影响的只有这种形态的候选,判据与全库测试见那边头注。
//
...
// {live} 大小不等吃 -600 —— 把同义词当成了独立的键。全库 4214 条决策回放:
...
//     纯命名不对称,只在时长≤1% + 专辑亲和验证同一次录音时豁免(那 12 条真不同的过不了时长门)。
//
...
// 所以这一类也得固定。新增打分项时把常量加进这个清单,忘了补译名就直接红。
...
//
// 之前这是直接判 -1(一票否决)。测试推翻了它:被判 -1 的候选里 83%(5/6)
...
//
// 落地的测试依据(处理「Stranger in Moscow (Tee's In-House Club Mix) 配了
...
// (sourceReportedDurationSecs 自 就在透传,注释写着"先攒评测数据")。
//
...
//
// 处理形状:六个播放器全勾(apple_music/auto/kugou/netease/qq/spotify),于是
...
func scoreLyricCandidateDetailed(
	...
	// consensusPeers>=1 兜底:(Gareth.T《before you》= 网易云/QQ/酷狗/
	...
	if durationSecs > 0 {
		...
		case last > durationSecs+lyricOvershootToleranceSecs:
			...
			// 跨源印证豁免:两个源一起匹配到同一个错版本恰恰是印证的已知未命中形态(Valentina)。
	...
	if isNativeLyricSource(c.source) {
		...
		// :用户放着 QQ 音乐听周杰伦《太阳之子》,这套打分给他配了酷狗那份
		// (酷狗有逐字 +400 直接压过 QQ),整行比 QQ 自己的歌词界面慢半个字左右。
		//
		// ⚠️ 这**不是**在把 删掉的那个"按来源加 10~50 分"改头换面加回来。
		...
		// 的跨源歌词,那正是 08-09 那次未命中的形态。250 的作用是在质量接近时扭转结果,
		// 不是碾压质量差距。
		add(scoreTermNativeSource, 250)
	}
	// ⚠️ 这里曾经按来源加 10~50 分。把它删掉了 —— 250 首抽样、751 条候选、
	// 以「跨源内容一致性」为准的消融实验结论:
	//
...
// 标题明显更吻合"。
//
// 起因是处理真实误配案例(方大同《公园 (Live版)》,收在专辑「大事发声·录音棚现场:
...
//
// (方大同 & Fiona Sit《Four Tour》案,「搜索候选歌词」弹窗"七个源都
...
// 连接两个拉丁书写艺人名"这一种写法。
//
// 补第二档——两侧都是中文时也可能是连接词。真实bug:陶喆《再見你好嗎》专辑
...
// scrobble 歌手名的写侧 —— 测试把 `K/DA/Madison Beer/i-dle/Jaira Burns` 折成了 `K`,
...
// 根本不存在的"K"反而能过**。
//
// 处理现象就是这个:「联网搜索候选歌词」用播放器给的完整歌手串只搜到
...
const (
	...
	// 两边**各自自报的曲目时长**——同一次录音跨平台只会差在取整。测试本案例
	...
	// 为什么需要:albumScore 的包含档对**短通用串**几乎免检——测试同一批酷狗搜索结果里
...
// artistAliasTable 没这条;MusicBrainz 那两条路(中文别名 / 主名)测试都返回空。
...
//
// 并且这条档位**只放行歌词候选,绝不放行身份/封面** —— 沿用 // withholdImpersonatorRiddenIdentity 那次确立的分层:歌词按曲目挂在各家的歌词库上,跟
...
func lyricRecordingTriangleMatches(candTitle, candAlbum string, candDurationSecs float64,
	...
	case sc >= 100: // 宽松包含 → 追加长度可比性要求
		...
		// 专辑那一整类。测试:本地专辑 "Editorial"(9 rune)对候选
...
//
// 加。真实案例:伊藤 美奈子《雨のメヌエット》(专辑 TENDERLY),**九个源零候选**;
...
// normLoose 只保留字母和数字、**空格本来就被它丢掉**(测试 `normLoose("伊藤 美奈子")` 与
...
// canonical_artist / 展示字段——把 "A & B" 缩窄成 "A" 正是 那次回归
...
// artistAliasTable 曾经是"已知英文/罗马化艺名 → 本库常用中文名"的手工对照表,// 到 08-30 之间从 5 条批量扩到过 23 条(用户逐条核对 Last.fm Top100 导出验证的同人异名)。
//
...
var artistAliasTable = map[string]string{
	...
	// 那个 Michael Jackson 真实bug修的,不能为了这一个人放松,见
...
// 按可信度排序、去重、不含原名。
//
// 加。在此之前重试只认 artistAliasTable 那 5 条手工登记 —— 而
...
// 补丁,而 musicBrainzArtistAliases(下面这条)双向通用、对任何歌手都生效(测试验证
...
// "罗马化名字→中文艺人"这个场景本来就是量身做的,测试同一批人绝大多数都能查对,包括
...
func retryArtistIdentities(ctx context.Context, artist string) []string {
	...
	// 第零点五条:这台机器上同一个歌手的**别的**歌成功解析时,源那边把他署成什么名。
...
// 把仿冒号的署名当成核实过的官方名。只需要极少数确凿知名的名字,新的按实际处理边界情况追加即可。
...
//	   测试把裸标题无条件提前之后,"Billie Jean (Single Version)" 的酷狗候选从 806 分
...
// 走 lyricTitleAccepted。刻意**不**把两种去装饰叠起来再多加一个变体:那是没有测试依据的
...
//
// (处理「这首歌找不到歌词」):D'Angelo 的 Voodoo 专辑里那首,Apple
...
// neteaseImpersonatorRiddenArtists 一个路子 —— 只收歧义极小的,新的按实际处理边界情况追加。
...
// ambiguousConcertMarkers 对齐**:那三张表回答的是**另一个问题**
...
// 不含这张词表。
//
// ⚠️ 「club」这个裸词有一个**测试过的**反例,别手滑加进来:同一张专辑上的「Earth Song」,
...
var distinctRecordingVersionTags = []string{
	...
	// 补的「…mix」家族(处理「Stranger in Moscow (Tee's In-House Club Mix)
	...
	// 补中文版本限定词(「周杰伦 - 蜗牛 (伴奏)」查 lyricfind,YouTube
	...
	// 粤语/国语/cantonese/mandarin **不在这张表里**:
	...
	// 补裸 "edit"(处理 PRINCE《Diamonds and Pearls (2023 Remaster)》配了酷狗
...
//
// 加 kuwo 时全库扫描酷我上有多个 DJ 把周杰伦/王力宏/林俊杰/方大同/
...
//
// ⚠️ DJ 名字不能只认中文:第一版用 [\p{Han}0-9.]*,测试漏了"稻香 (完整版|DJ Ray版)"
...
// 是正式版(同理),都算不匹配。
//
// **歌名和专辑名一起看**。验证这一点非做不可:PRINCE 的 "1999"
...
//
// 起因》案):QQ 音乐给 The One 演唱会里的曲目起名就叫
...
// 全库回放匹配到的真实误伤:韦礼安《女孩》的 lrclib 候选专辑名
...
const (
	...
	// 补的三个跨语言键(处理「为什么这首歌匹配错版本了,匹配成英文版本了」——
...
// 两个都看,金标集当场匹配到反例:陈奕迅《Shall We Talk》是粤语歌,本地却收在《陈奕迅 国语精选》
...
func inferLocalLanguageVersion(localTitle, localAlbum string, durationSecs float64, candidates []lyricCandidate) string {
	...
	// ②专辑精确同名 **且自报时长与本地 ≤0.5% 吻合** 的声明候选。时长这道门不能省:全库回放匹配到
...
//
// 目前只有 "single version"。它跟 album version 不同,不能直接移出词表:
...
// 专辑亲和**这两道硬证据验证同一次录音时豁免 —— 那 12 条真不同的过不了时长门,照罚。
// 测试:11 条误罚因此取消(Jackson 5《Never Can Say Goodbye (Single Version)》对 QQ 裸标题
...
// sameRecordingDespiteVersionTags 判定"版本限定词对不上,但其余证据验证这就是同一次
...
// 演出」。600 也是测试出的必要量级:陈奕迅《Shall We Dance (Live)》案冠亚分差 419
...
//	   《Queen Rock Montreal》是**同一场**蒙特利尔演出,全库回放验证这一档必须挡(3 条
//	   Queen 假阳性全靠这道门排除);
...
//	   QQ/酷狗给中文歌普遍缀英文别名("起源" vs "起源 Origin",这首歌
//	   两个源都因此被拦,五源只剩两条候选)。见 bilingualTitleEqual。
//
// **绝不认任意的双向子串包含**。那是 之前 kugou/QQ/Musixmatch/netease 的
...
//
// 收紧的代价测试为**零**:250 首全量候选数据里,「旧规则(子串)认、新规则不认」的候选
...
// 判,比"括号里的字对不对得上"准得多。
//
// 历史:这里曾经有一个「歌名匹配:忽略括号/严格」的用户设置。后删除
//
...
func lyricTitleAccepted(candidateTitle, localTitle string) bool {
	...
	// (见 stripStructuralTitlePrefix 的测试记录)。
	//
...
//
// ⚠️ 返回**名单**而不是计数(借鉴清单 V2,):两两 Jaccard 本来就在下面这个双重
...
// (决策 49 的消融测试:行数项翻盘 57/57 都是 right→right、60% 的翻盘对 Jaccard>=0.90),
...
// "只接受 sourceMessage 标 LyricFind 的候选"把 Musixmatch 挡在 lyricfind 之外;// 接 deezer 时从**另一个方向**踩进同一个坑——不是"一个源里混进别家数据",而是"两个源来自
...
func contentConsensusPeers(localArtist, localTitle string, candidates []lyricCandidate, durationSecs float64) map[string][]string {
	...
	for _, c := range candidates {
		// ⚠️ 修复的一部分:这里**不再**把 isProbablyWrongLanguageLyrics
...
// 样本验证前,负档是死代码兼潜在误伤源,见 评测目录的裁剪记录)。
...
// 它的 +400 撤掉(K歌之王案测试:免罚后 1123→723,仍输给 867 的网易云)。false 时等于 v14。
...
// 。而 feat. 名单恰恰是 titleMatchTierPoints 最该判成"纯噪音括号、
...
func usableValueAdd(lyrics, tr, trLang, roma, targetLang string) (usableTr, usableRoma bool) {
	...
	// 价值的场景(日文歌 + 网易云中文对照)。用假名占比把日文形态摘出来。
...
//     后者以前者开头就算同一首(这正是 删掉的"双向子串包含"的定时炸弹,
...
// 前半段有效果后面突然没了"。
//
// 阈值 0.5 是照测试数据定的:扫过本机 136 条有逐字的缓存条目,被截断那条覆盖 19.1%,
