...
// 一遍文件)。复现(ROSÉ & Bruno Mars《APT.》):amll 候选构造那一步早就
...
func runResyncLyricsCLI(args []string) {
	...
	// 标题反查轮每次都要现查一遍 MusicBrainz——验证:这个缺口导致
...
func runResyncLyrics(keys []string, apply bool) int {
	...
	for _, key := range keys {
		...
		// (温岚《夏日の風》,本地标签是繁体"溫嵐 (Landy Wen)"/"溫式效應"):
		...
		// 验证(陶喆《Airport in 10:30》):这条 CLI 手上就攥着 enrichCache
