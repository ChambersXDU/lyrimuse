...
// 清空 —— 图有用,但不认领归属。可 之前,`siblingAlbumCover` 把这样一张图借给
// 同专辑其它曲目时,调用方会盖上 `cover_album = album`:一次借用把"未认领归属"升级成了
// "逐字对上专辑"。两层后果:App 侧错图顶掉 Last.fm 那张对的图(处理
...
// **已经盖下去的戳**擦掉:本机测试 386 条(占缓存 4068 条的 9.5%)。
//
// # 为什么只擦戳、不顺手把封面换掉
//
// 这里完全能顺手做:`siblingAlbumCover` 是纯内存扫描、不发请求,测试那 386 条里 79 条同
