...
const (
	...
	// 地址在当前网络下直连不通(测试 100% ICMP 丢包、TCP 16 次只成功 3 次、TLS 16 次
	...
	// 测试见过的失败模式 —— "这首歌没有歌词"(GraphQL 的 LyricsNotFoundError)是正常
...
// 传输层通用代码(,处理「为什么这首歌搜不到」:九个源里六个在 DNS 解析这一步
...
const (
	...
	// 绝不是"查过了没有"。评审时匹配到的漏洞:不给它归因,弹窗会把它算进「其余 N 个源」,而
