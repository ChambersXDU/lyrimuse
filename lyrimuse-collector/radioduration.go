...
// 电台曲目的**真实曲长**补进歌词缓存。
//
// 为什么要单开这条路:电台快照里的 duration 是整档节目(测试 3390.122s),真曲长只有 Apple 目录知道
// (appleCatalogAnchor,测试把 3390.122 纠成 226.283 / 131.173 / 237.800)。而目录锚点是**异步**的:
// 一首歌刚换过来那一拍通常是缓存未命中、后台才去取,偏偏歌词条目就是在那一拍写下的 —— 所以条目里
// 的时长是空的,几秒后目录到位了也没人回头补。测试:Tame Impala《Borderline》条目 09:18:46 落盘,
