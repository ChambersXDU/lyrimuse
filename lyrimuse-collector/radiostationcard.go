...
//
// 电台起播时系统会先把**台名当一首歌**推过来,持续几十秒(测试 开台到第一首歌之间
// 33.4 秒)。形态是 title=台名、artist 空 —— 测试三例:`|petal radio|`、`|NCT 127|`、`|YEONJUN|`
// (enrich key 是 `歌手|歌名|专辑`,所以第一段空的就是没有歌手)。
//
// 不拦的话它会被当成一首歌:去网络搜一轮歌词、搜不到、再把这条空壳永久写进磁盘缓存,污染
// 「歌词管理」列表 —— 用户 报的就是这个(「第一次开始播放一个电台时……还是会被当成
// 一首歌去搜索,然后显示"暂无歌词"」)。发现时缓存里已经攒了 6 条,无一例外没搜到歌词。
//
// 判据只用「电台 + 没有歌手」。真曲目两样俱全(测试所有真歌都带歌手),而缺歌手时歌词搜索本来
