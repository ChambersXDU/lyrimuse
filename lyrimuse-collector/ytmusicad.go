...
//
// 那道守卫是 定的,判据是四份真实样本里 album 唯一 100% 分得开歌与视频;注释
...
// 读到的专辑名**。判定留给 Go 侧(parseYTMusicAdProbe),这样
...
//
// 。起因是处理「YouTube Music 播一张专辑的时候,第一首歌怎么不上送专辑名」。
// 当场抓的测试(两张不同专辑、四个采样)验证那是 **YouTube Music 自己的疏漏**:它开一条新
// 队列时在专辑上下文解析出来之前就把 MediaSession 元数据设好了,之后不再刷新这一首,所以
// 页面 byline 上后来有了专辑名、MediaSession 里那份却永远停在空;第二首起就带上了。
// (完整的四行测试表在 Swift 侧 `YouTubeMusicAdProbe.albumPatch` 的注释里,含那条证明
...
//   - `tell application id "<bundleID>"` 而不是写死 App 名字(测试对三家 Chromium 系和
...
// 测试 ~0.9s(见 Swift 侧 BrowserPositionProbe 的测试记录),每 5 秒烧 0.9s 不值当。
//
// 按曲目身份缓存对**音频歌曲**是安全的:广告在 media-control 里是一条**独立的 now-playing 条目**
// (自己的 title/artist/duration,测试如此),换成广告身份就变了、缓存自然失效。
//
// ⚠️ 对**音乐视频(MV)的前贴片广告**不成立:
...
//
// ⚠️ **但"两边同时改"只管判据,不管这两个缓存常数**。Swift 侧的
// 歌档 是 45 秒、不再是 60:那边 `kickIfNeeded` 是**异步**的(同步路径不能被一次
// AppleScript 往返卡住 UI 轮询),再探间隔一旦等于可读有效期就没有重叠窗 —— 判定过期那一拍必然
// 「读缓存拿到 nil + 这一拍才开始重探」,fail-closed 把快照整条丢掉、三个展示面一起塌成"没有在
// 播放";真机日志验证一首 album 为空的歌每 60 秒塌一次。**这里不需要跟着改**:下面
