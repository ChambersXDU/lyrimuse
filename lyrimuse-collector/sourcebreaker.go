...
// 代码报给弹窗,那三个是 的,见 lyricsourcefailure.go。
...
// 实际请求的域名;raw.githubusercontent.com 全仓只有 amll 在用。
...
func (b *lyricSourceBreaker) observeWith(host string, err error, status int, retryAfter string, tr transportTrace) {
	...
	case err != nil || status >= 500:
		...
		// 时它们在同一瞬间一起失败,于是一次抖动就能把阶梯从头走到尾。日志:
...
//
// ,处理「派对后派对(黄妍)搜不到」。真相:这台机器连着公司 OpenVPN,它下发的
// DNS(10.255.0.1)对 music.163.com / c.y.qq.com / mobilecdn.kugou.com / lrclib.net / search.kuwo.cn /
// pd.musicapp.migu.cn 一律不答(dig 测试:多数超时、偶尔空答),六个源的请求 2ms 内就以
...
//
// ⚠️ DNS 失败怎么认(评审时匹配到的坑):不能只靠 errors.As(err, *net.DNSError)。各源的 client 都
