...
// (MediaRemote 的 kMRMediaRemoteNowPlayingInfoUniqueIdentifier)。验证:
...
//     任意的 64 位本地持久 ID,可以是负数——测试同一台机器上一首本地导入的 Michael
...
func appleCatalogAnchor(bundleID string, trackID int64, localTrackNumber int, localTitle, localAlbum string) (appleCatalogTrack, bool) {
	...
	// ⚠️ 曲目名用**逐字同名**,不是 lyricTitleAccepted。
	...
	// 音轨号交叉核对:逐字同名也挡不住**完全同名**的兄弟轨(测试 XSCAPE (Deluxe) 上
...
//     测试:「枫+退后+搁浅 (Live)」本地署名「南拳妈妈弹头」时网易云 4 条查询词一条都
...
// **查到了**的条目,查空的只留在内存里(避免一次偶发的网络抖动/超时把这张专辑永久固定
// 在"没有别的署名"上,下个进程还有机会重试)。,起因是把 retryArtistIdentities
...
// appleStorefrontArtistCacheVersion:落盘格式版本。v1 是裸 map,里面的署名**没有经过
// 「这首歌确实在那张专辑里」的核对**(见 appleStorefrontTrackMatches),已测试装进过错人(back number《Happy End - EP》
...
//
// ,测试验证过(curl 直接打 iTunes Search API,不是猜的):查询词全程用
...
//
// ,真实病根(处理「为什么这首歌只搜出这一个结果」,Mrs. GREEN APPLE《クスシキ》):
...
//     专辑名,而 YT Music 的 zh-HK 界面把艺人名**本地化**了(处理王子
...
//
// 查询词先用「艺人 + 曲名」(iTunes 对认不出的艺人 token 容忍度不错,测试「王子 Why You…」
...
const (
	...
	// 几十秒的时长在 iTunes 里最容易撞上另一位艺人的同名段(测试 陶喆《Doxology》47s 撞出
