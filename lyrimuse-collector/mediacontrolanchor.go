...
// 对 Spotify 准到毫秒级(换歌后首拍读数 0.33~0.37s,正好是通知+去抖的延迟)。
// rate 缺失/为 0 时 elapsedTimeNow **不再外推**(:Spotify 暂停后恢复播放
// playbackRate 变 null;复测 elapsedTimeNow 2 分 14 秒纹丝不动),只能自己按
// elapsedTime + (now − 锚点时刻) 补算。而 timestamp 恒无小数,直接拿它当锚点时刻会恒偏快
// frac ∈ [0,1)(测试 .914/.724/.560),暂停一下就退回去,还把下一首自然切歌的偏置估计带歪。
...
// 同一套判据,两侧必须同时改。完整测试记录见那边的注释与 docs/features/02。要点:
// Spotify 会在播放中把 now-playing 信息重发一遍,elapsedTime **逐 ms 不变**、时间戳却换成
// 当下(测试 10.477@:09 → 10.477@:43),MediaRemote/media-control 据此外推的位置一下退回
