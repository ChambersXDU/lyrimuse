...
// 存量条目怎么被重新解析一次:cover_album 是 才加的字段,老条目一律为空,
// 只能靠补一次重解析来判定 + 写上。
...
func TestCoverSwapAllowed(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// (Michael Jackson《Workin' Day and Night (Immortal
...
// 处理《Michael》/「Hold My Hand (with Akon)」封面不对:显示的是 QQ 的
...
// 借拿到 device 来源,而它的归属是测试证据,不该再被"网易云这一轮应答过没有"那条代理证据拦住。
