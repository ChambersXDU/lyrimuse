...
// 逆向出来,用 curl 测试两个端点全部验证过——
// 不是照抄一份没验证过的第三方代码,是照抄一份**验证过真能用**的接口契约。
//
// ⚠️ 这个源的搜索排序完全不可信,这是接入前的,不是猜测:兰亭序/周杰伦、
...
// 歌词,第一份真的带同步时间戳的才采纳。按这套流程测试的产出率:海阔天空 5 中 4、
...
type kuwoResult struct {
	...
	// cover:。搜索结果自带 web_albumpic_short,不用像 kugou 那样再多发
...
type kuwoSearchItem struct {
	...
	// WebAlbumPicShort:验证的封面字段,形如 "120/38/70/3416909732.jpg"——
...
// 各自要求不同的 Referer,写死同一个会被其中一个拒掉,验证)。
...
// 文件头注"搜索排序完全不可信"那段测试。曲名/歌手用跟别的源完全一致的判定函数
...
// "有没有真同步歌词"才是筛不掉的那道门,拉太多条只是白耗网络,5 条足够覆盖测试产出率
