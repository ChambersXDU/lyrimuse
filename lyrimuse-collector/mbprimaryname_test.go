...
// mbAliasCandidatesForRetry 的判据(,当时叫 mbPrimaryNameForRetry、只给
// 一个"主名"字符串;改成给全部已登记写法,见其声明处头注)。
//
// 起因是真实反馈:《Hurry Up Tomorrow》里的「Cry For Me」搜不到歌词。测试原因是 Apple
...
// primary=false,反而是从没用过的日文别名 primary=true,这正是 那次改动
// 不能按 alias.Primary 过滤的测试依据),不是编的。
...
// 就返回空;要是把空也永久写进文件,一次偶发限速会把这位歌手永久固定在"没有别名"上,
