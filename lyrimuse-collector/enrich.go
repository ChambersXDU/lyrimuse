...
// needsRomanizationRetry determines whether to retry search with artist name variants
// (e.g. native script vs romanized stage name) to obtain romanization or language signals.
// For example, if initial candidates return English/Romanized text without language tags,
// querying native script variants allows QQ/Kugou to return language signals (e.g. Cantonese)
// or NetEase to return romanized lyrics.
//
// Checks dominantScript across candidate lyrics for Han, Kana, or Hangul scripts.
// If any source already provides romanization or language signals, retry is bypassed.
...
// maybeGenerateJyutpingRoma generates Cantonese Jyutping romanization LRC when song language
// is Cantonese and no source provided romanized lyrics. Only populates when LyricsRoma is empty
// and never overwrites existing source-provided romanization.
...
type enrichEntry struct {
	...
	// QQ Music album mid and primary singer mid for navigation in the lyrics window
	// ("Go to Album" / "Go to Artist"; see qqSongCatalogMids).
	// Omitted from fields() payload to conserve relay/LB bandwidth.
	...
	// Romanized lyrics (line-by-line LRC). Three sources in order of priority (see maybeGenerateRoma):
	//   1. Source-provided (NetEase romalrc, QQ roma, Kugou KRC).
	//   2. Cantonese Jyutping table-lookup generation (maybeGenerateJyutpingRoma).
	//   3. Helper process romanization for Japanese/Korean/Chinese (maybeGenerateHelperRoma).
	// Lower priorities only populate when this field is empty, never overwriting source-provided data.
	LyricsRoma string `json:"lyrics_roma,omitempty"`
	// PlainLyrics stores non-timestamped plain text lyrics used as a display fallback
	// when synchronized lyrics are unavailable. It is stored separately from Lyrics (LRC)
	// to prevent synchronized rendering engines from treating plain text as malformed sync data.
	//
	// Written via two paths:
	//   1. Explicit user selection of a plain text candidate in UI ("Adopt as Plain Text").
	//   2. Automatic fallback in rescoreLyrics/resolveEnrichAsync when no synchronized candidates
	//      score >= 0 and a PlainTextOnly candidate exists.
	// Declared explicitly in Go struct so json.Marshal preserves values saved by Swift.
	PlainLyrics string `json:"plain_lyrics,omitempty"`
	// PlainLyricsSource records the source provider for PlainLyrics (e.g. lrclib, musixmatch).
	...
	//
	// Multi-source search has a deadline (lyricSearchDeadline). Slower sources that may carry
	// word-by-word synced lyrics (such as NetEase) might not return in time for the initial resolution.
	// Recording seen sources and score allows needsLyricsRetry to re-evaluate when conditions permit.
	...
	// LyricsDecisionApplied stores the evaluation record corresponding to the currently active lyrics.
	// Kept in a separate slot from LyricsDecision (which logs the most recent evaluation attempt)
	// so details accurately reflect the decision that applied the active lyrics.
	...
	// ResolvedDurationSecs records the track duration used when selecting lyrics.
	// If the actual playback duration significantly mismatches this value (e.g. different
	// edits or album prefetch duration differences), lyrics are re-selected against actual duration.
	...
	// LyricsRescoreVersion records the scoring rule version targeted by previous attempts.
	// Limits and throttling apply per scoring version, resetting when the scoring version increments.
	...
	// DurationSecs stores actual track duration for consistent scoring between automated
	// matching and manual search in the lyrics management interface.
	...
	// ManualLyrics marks lyrics manually corrected or explicitly adopted in the lyrics management UI.
	// Serves as an absolute veto against automatic retry or rescoring, preserving manual edits.
	...
	// ManualPickSHA is a passthrough field preserved across cache serialization round-trips.
	// Holds content hash from manual lyric adoption (see LyrimuseCore/ManualPickLock.swift).
	// Must remain declared in Go struct so encoding/json does not drop it during marshal/unmarshal.
	ManualPickSHA string `json:"manual_pick_sha,omitempty"`
	// Instrumental flags tracks confirmed as instrumental by at least one source (e.g. lrclib instrumental
	// tag or NetEase pureMusic marker), distinct from general missing lyrics.
	Instrumental bool `json:"instrumental,omitempty"`
	// TS records the timestamp when the entry was initially resolved.
	// Used by needsLyricsRetry as baseline for retry interval.
	TS int64 `json:"ts"`
	// PeripheralTS records the timestamp of the last peripheral backfill attempt,
	// separated from TS to ensure peripheral throttling does not delay lyrics retry.
	PeripheralTS int64 `json:"peripheral_ts,omitempty"`

	// SpotifyTrackID stores the 22-character track ID from AppleScript for Spotify tracks (see spotifytrack.go).
	// Used by spotifyLink() to construct canonical track URLs open.spotify.com/track/<id>.
	SpotifyTrackID string `json:"spotify_track_id,omitempty"`

	// Unknown stores unmodeled JSON keys to preserve them across MarshalJSON round-trips (enrichjson.go).
	// Prevents binary version mismatches from stripping fields written by newer components.
...
// 多了一个例外:命中的既有条目时长跟当前差出这个阈值,说明 key 撞车其实
...
//
// 从"只差大小写"扩到"大小写 + 空格 + 繁简"。起因是用户在「歌词管理」里看到
// 成对的重复,全库 108 条测试有 14 组 29 条:
//
...
//
// ⚠️ 光有 canonicalEnrichKey 挡不住重复,这是 被真实数据后补的:
...
func loosenEnrichKey(key string) string {
	...
	// `VALORANT & Grabbitz & bbno$`。测试缓存里因此长出 12 组、24 条只差分隔符的重复
...
func trackEnrichment(ctx context.Context, artist, title, album, bundleID string, durationSecs float64, isNewTrack, radio bool) map[string]string {
	...
	// 电台台卡不能拿去搜歌词。开台那几十秒系统把**台名当一首歌**推过来:测试 title=台名、
	...
	// 完整理由(含 那个"全勾之后三个源同时 +250"的真实 bug)见 match.go 里
	...
	// 重试/重打分)也要用它——(林潔心《想逃避(22)》):enrichKey 内部
	...
	if !ok {
		...
		if alt, found := canonicalEnrichKey(key); found {
			// 只在第一次撞上这个 key 时记一行:这条路径每次轮询同一首歌都会走到,
	...
	if ok {
		...
		if isNewTrack && e.CoverSource != "device" && !enrichInflight[key] {
			...
			// 第二个条件:播放器没报专辑、回填出的专辑名跟现有封面完全不沾边 —— 首次解析时
			// 没有专辑名可用、Apple 第一条合集就此冻结,这条给它一次按回填专辑重选的机会(见 coverNeedsHintCheck)。
			//
			// 第三个条件:动态封面是新加的字段,存量条目一个都没有 —— 跟第二条同构地挂在
	...
	if _, busy := looseInflightKey(key); !busy {
		...
		// enrichcancel.go 和 resolveEnrichAsync 的注释。继承上层 lifecycle ctx,
		// 并在用户在"歌词管理"占位行点"停止搜索"时单独取消该 key。
...
func needsPeripheralBackfill(e enrichEntry, artist, album string) bool {
	...
	// URL 非空 —— 于是那批条目**永远不会**再被补一次(本机测试 565 条里 40 条卡在这一档),
	...
	// **故意扣掉**的,补多少轮都不会有 —— :周杰伦 218 条 netease_url 全空,其中 42 条
	...
	// 封面归属可以升级成"借同专辑一张测试证据图"时也算缺:coverNeedsAlbumCheck 只查网易云那一档,
	// 而这次要救的正是它刻意不查的 qq 档。
...
// needsPeripheralBackfill 与 coverNeedsHintCheck(albumhint.go)两条触发条件共用 —— 从前者尾部拆出来,
// 行为不变。
...
//   - cover_album 为空(老条目,这个字段 才加)→ 判不出来,补一次重解析顺便
//     把这个字段写上。
//
// ⚠️ 从 `== 0` 改成 `< 200`(收严):albumScore 的 100 分档是"宽松包含"
// (见它自己的注释——"几乎任何同名重发/纪念版都会被判定为'包含'、拿到 100 分"),对
// "同一个基础专辑名、不同版本封面确实不一样"这类情况(方大同「很不低调」「烦」
...
func coverSwapAllowed(old, fresh enrichEntry, album string) bool {
	...
	// **不再成立**:同专辑邻居那一档现在可以借走一张 device 封面(见
	// siblingAlbumCover 第一档),fresh 因此可能是 device 来源。下面单独加了一档处理它。
	//
	// (同一首《Immortal》):Michael Jackson 这类不需要中文别名的歌手,
	...
	//
	// ⚠️ 这条**不再是无条件拒绝**:低分辨率的设备封面(浏览器 MediaSession
	...
	// cover_album 已经逐字对上这张专辑 —— 归属是测试证据、不是文字匹配,按全系统的可信度
	...
	if fresh.CoverSource == "qq" {
		...
		// 补:这一档现在也可能是"网易云/Apple 都给了封面但都对不上本地专辑
...
//
// 处理方大同《JTW 西游记 (Gold) [Explicit]》「Once」这一档存在
...
// 原有行为。
// ⚠️ **分两档,并回传"这张图的归属够不够格声明"**(第三个返回值)。起因是
// 处理《Michael》那张专辑里「Hold My Hand (with Akon)」封面不对:实际显示的是 QQ 的
...
//     这条记录从此**永远不会**再被复查。本机测试这样被盖过章的条目有 386 条。
//
...
// (重发/纪念版换了封面)照样能逐字对上,借过去等于把一份靠文字对上的答案当成测试证据
// 传染给另一首歌 —— 跟下面那段"不借网易云/Apple"是同一条理由,那边说的是 100 分档。
// 本机测试:386 条被盖章的条目里 79 条同专辑有 device 邻居可借、122 条只有 netease/apple
// 已核实邻居、185 条一个可借邻居都没有。后两类靠"不再盖章"就已经回到正确行为(自带图赢),
// 要不要把 200 分的 netease/apple 也纳入借用是另一个独立取舍,没有测试依据前不做。
...
// 来源算测试证据"只有一处定义 —— 以后真有第二个来源够格(比如按专辑 mid 而不是专辑名
...
//
// 为什么不直接放进 coverNeedsAlbumCheck:那个函数**只查网易云那一档**是一条有测试理由的
...
//
// 一开始写的是"所有启用的源都回来了才算数",当天就被真机日志:五源搜索
...
// 不可能成功——用户看到的是「这一轮「」没应答」这种主语为空的话。测试案例
...
// 的处境,不靠巧合。(顺带测试过:294 条去重缓存条目里,"有歌词但没记 lyrics_source" 是
...
// 存量失败就又变成永久失败了,而那正是 处理「这首歌找不到歌词」时发现的问题
...
// 之外,**一首歌解析失败过一次就永久卡住**。验证:修好 `Medley: ` 前缀那个
...
func needsLyricsFirstFill(e enrichEntry) bool {
	...
	// 这是原有的兜底);都不冷却了就只等 30 秒。加后面这一档,起因是处理
...
//
// 验证的串扰:换曲/预载窗口里 media-control 会把**下一首**的时长和当前
...
// 盖掉(那次事故正是处理"决策记录怎么跟手动搜索对不上"的根源)。
//
...
// 连发重试把预算一口气烧光。
//
// nowUnix 由调用方传入而不是自己取 time.Now:去抖语义全靠时间差,单测要能固定。
...
// 观察断流的陈旧上限。堵的是"跨播放残留":脏快照落在曲目**切出**
...
func needsLyricsRetry(e enrichEntry, wrongDuration, pinned, autoUpgrade bool) bool {
	...
	//
	// ⚠️ 这一条必须排在下面"已经有逐字就不重试"**之前**。:用户放 QQ 音乐
	...
	// 用户手改过的绝不自动重搜,理由见 ManualLyrics 字段的注释。这道闸 补上:
...
func retryLyricsUpgrade(ctx context.Context, key, artist, title, album string, durationSecs float64, firstFill bool) {
	...
	// 界面永远看不到。处理"译文语言切成英文了还是没有翻译",日志里译文明明
...
//
// ⚠️ 上限是**每个打分版本** 3 次,不是终身 3 次:
// 版本升了计数从零算,不然连升几版之后存量条目会一个个被永久冻结、开关形同虚设。
//
// 节流是 当天补的:原来只有次数上限、没有时间间隔,以为"次数兜得住"。
...
func rescoreLyrics(ctx context.Context, key, artist, title, album string, durationSecs float64) {
	...
	default:
		...
		if picked.Source != e.LyricsSource {
			...
			// LyricsSource 静默改回旧值,rescore 的结论被回滚。
			lyricsChanged = true
		}
		e.LyricsSource = picked.Source
		e.LyricsScore = picked.Score
...
// goroutine 有界、进程退出即止。
// resolveEnrichAsync 首次解析一首歌——加了取消支持(见 enrichcancel.go):
...
func resolveEnrichAsync(ctx context.Context, key, artist, title, album, bundleID string, durationSecs float64, isNewTrack bool) {
	...
	if ctx.Err() != nil {
		...
		// 节奏(24h 起始指数退避),不是永久固定在"没有"上——取消只是让用户不用
		// 干等这一次,不是给这首歌下了永久结论。
		//
	...
	if e.CoverURL == "" && e.Lyrics == "" && e.AppleURL == "" && !hasRealQQURL && e.NeteaseURL == "" {
		// 全空分两种,结论完全不同,之前被压成同一个"不写"(处理王子(=Prince)
		// 《1999 (Edit)》"一直显示搜索歌词中,它没有一个时间上限吗"):
		//
		//  · **根本没查成**(断网/整轮一个请求都没发出去)——不写是对的,下次再试,别把一次
		//    网络抽风固定成"这首歌没歌词"。这是上面那条守卫当初存在的全部理由,原样保留。
...
//
// ⚠️ :顶替**不再无条件** —— 低分辨率的设备封面(浏览器 MediaSession 常给
...
// 验证:批量修完 18 条之后,`Queen|Another One Bites The Dust`(当时正在
...
func backfillPeripheralFields(ctx context.Context, key, artist, title, album string, durationSecs float64) {
	...
	// 存量 QQ 封面提档:之前拼的 QQ 封面 URL 写死 300x300,
	...
	// 列出来,算出来的值就在函数返回时丢掉了,缓存里 4820 条一条也补不上。
...
func resolveTrackEnrichment(ctx context.Context, artist, title, album string, durationSecs float64, deviceCoverURL string) enrichEntry {
	...
	// 只要网易云在「歌词来源」里开着就查一次——封面/跳转链接搭它的车。⚠️ 用户
	...
	// "启用哪些源"过滤,理由见它的注释。
	//
	// 删掉了「歌词在线匹配」总开关(见 features.go 的说明),原来关掉时走的
	...
	//   加,修的就是同一个歌手有的曲目匹配成功、有的失败这个问题);
	// ②resolveGenericArtistCanonicalName(见下面那处调用)。
	//
	// ⚠️ 删掉了原来夹在中间的两级:「网易云本次搜索这首歌带回的歌手名」
	...
	// 请求)——排查验证(Michael Jackson《Morphine》,本地专辑标签是
	...
	// 见 apple.go 注释)明显更强,而且这份数据本来就要为跳转链接查一次——测试这首歌
	...
	// 封面解析用的专辑名:播放器没报专辑时用 Apple 目录按「署名 + 曲名 +
	...
	//
	// 验证(蔡徐坤《KUN》11 首):网易云整张专辑都没有,只有 Deadman /
	...
	// 新版。处理方大同「很不低调」「烦」就是这个缺口,而且第一版这里只
	...
	if e.CoverURL == "" || (coverAlbum != "" && albumScore(e.CoverAlbum, coverAlbum) < 200) {
		...
		if qqCover != "" {
			...
			// e.CoverAlbum 已经写着网易云那个不对版专辑名的状态换过来的,留着旧值会让 cover_album
		...
		// 跟 ne.Artist 那级一起删掉了(理由见上面 canonical_artist 解析链路
	...
	// 不够精确的候选"更可信。见 siblingAlbumCover 的注释。
	if coverAlbum != "" && albumScore(e.CoverAlbum, coverAlbum) < 200 {
		if url, source, albumVerified := siblingAlbumCover(artist, title, coverAlbum); url != "" {
			...
			// ⚠️ cover_album 只在**借来的那张图自己就核实过归属**时才盖:借一张不认领归属的 qq 图、却盖上本地专辑名,
	...
	if deviceCoverURL != "" {
		...
		//
		// ⚠️ 这一顶**不再无条件**:浏览器 MediaSession 给的封面常常只有
		// 120×120(见 deviceArtworkMinEdge 那条注释,下限故意压到 64 就是为了不漏掉它们),
		// 而歌词窗口那张大卡要画到 ~560px —— 处理「这首歌封面这么糊」(《24K Magic》,
	...
	if e.CanonicalArtist == "" {
		...
		// title/album 本身就跨语言对不上文本的 feat. 曲目)——改用
...
func (e *enrichEntry) fillMotionCover(ctx context.Context, title, album string) {
	...
	// 同一张(测试跨专辑指纹距离 19～34,阈值 10),会被当场拦掉。这是 用户提的
...
type scoredLyricCandidateResult struct {
	...
	// SourceReportedDurationSecs:源自己声明的曲长(秒),0=该源没给。透传,
	...
	//
	// ⚠️ 这里原来写着"LRCLIB 没有封面这个概念,QQ 这条路径也没查封面"——
	...
	//
	// 加,起因是一次**事后无法审计**的错配:打上花火被反查成《春雷》、整屏歌词
	...
	Instrumental bool `json:"instrumental,omitempty"`
	// PlainTextOnly:加,见 lyricCandidate.plainTextOnly 头注——true 时 Lyrics
...
// 验证这条错误注释掩盖的真问题:「悟空 2003 Demo」连查两次,一次 3 秒返回、
...
func scoredLyricCandidatesStreaming(ctx context.Context, artist, title, album string, durationSecs float64, onUpdate lyricSearchUpdateFunc) (neteaseInfo, []scoredLyricCandidateResult) {
	...
	// 搬运频道形态的身份重入:Safari 播 YT Music 里「音樂頑童」
	...
	// 测试同一首换成「Musiq Soulchild / Buddy」四个源立刻命中(QQ 1102 / 酷狗 1092 / 网易云 765 / LRCLIB 647)。
	//
	...
	//
	// 加第二个触发理由 needsRomanizationRetry(側田/Justin Lo《Erica》真实
	...
	if rescue || romaRetry || len(missing) > 0 {
		...
		for _, alt := range altIdentities {
			...
			// 中间态闪变——用户测试复现:原名一轮先展示出 LRCLIB 那条候选,
			...
			// ⚠️ (海龟先生《Porn Star》,「搜索候选歌词」弹窗"有时候
	...
	if primary := lyricPrimaryQueryArtist(artist); primary != "" && usableLyricSourceCount(results) < targetSources {
		tryVariant := func(alt string) {
			...
			// (isProbablyWrongLanguageLyrics,除了看 localArtist/localTitle
	...
	if usableLyricSourceCount(results) < targetSources {
		...
		// 见 retryArtistIdentities 头注): "Khalil Fong 梦想家 The Dreamer" 网易云
		// 专辑搜索零条,换成 "方大同 梦想家 The Dreamer" 才能搜到——所以搜专辑这一步也要用
		// 已知别名,不能直接拿原始艺人名去搜。
		//
		// ⚠️ (方大同《Lovers Policy》案,「搜索候选歌词」弹窗反复报
		...
		// 误差更小的那个(诊断日志验证过:直接测「Khalil Fong」两条兜底都 200 成功、
		...
		if correctedTitle != "" && normLoose(correctedTitle) != normLoose(title) {
			...
			merged := mergeLyricCandidateRounds(artist, title, album, durationSecs, results, altResults)
			// (Karencici《Hard to say》,「搜索候选歌词」弹窗):这里以前只在
...
//
// ⚠️ 补上 language / plainTextOnly:之前漏拷,后果是走过别名轮/变体轮合并的
...
//     误杀——变体串只作检索词和源内采纳闸,绝不进打分。⚠️ 这道语言闸多了
...
func mergeLyricCandidateRounds(artist, title, album string, durationSecs float64, base, extra []scoredLyricCandidateResult) []scoredLyricCandidateResult {
	...
	// "合并后有没有**标记那个源自己**的真候选"。从写死的 lrclib 改成按标记
...
// 没有可靠的封面字段、LRCLIB 没有封面这个概念,但 iTunes Search 曲库覆盖面很广(测试
...
// goroutine 各自往 resultsCh 送的就是它。从那个函数的局部类型提到包级,理由只有一个:
...
type lyricSourceResult struct {
	source                  string
	ne                      neteaseInfo
	lyr, yrc, tr, roma      string // roma:源自带罗马音(逐行 LRC),目前只有 qq 走这里(网易云的在 ne.Roma)
	...
	// 网易云的 pureMusic/占位正文、QQ 的占位正文、
	// musixmatch 每行都带的 instrumental 字段。
	instrumental bool
	// plainOnly:加。lrclib 与 musixmatch 两个源会给(musixmatch // 接入,见 resolveMusixmatchLyric 里的纯文本回退)——语义见 lrclibResult.plainOnly 头注。
	plainOnly bool
...
func rankLyricSourceResults(artist, title, album string, durationSecs float64, raw map[string]lyricSourceResult) []scoredLyricCandidateResult {
	...
	// coverOrFallback:候选自己的源有封面就用自己的——网易云/QQ/酷狗/
	...
	if lrclibLyr == "" && lrclibInstrumental {
		...
		// QQ 那一路。测试案例:蛋堡《收敛水》第 1 轨「关键字: Intro」
	...
	for _, c := range candidates {
		...
		case "netease":
			...
			// 那次计划里"刻意不做的"项,QQ 那一半 已经补回,见下面 case "qq")。
			// "固定中文"这件事必须记下来:目标语言不是中文时,
			// 这份译文用不上,得让机翻接手(见 needsTranslationBackfill)。
			//
			// ⚠️ (温岚《Jumping Machine (跳楼机)》,繁体原文配了一份
		...
		case "amll":
			// 复现(ROSÉ & Bruno Mars《APT.》):amll.tr 早就在
		...
		case "qq":
			// 加。QQ GetPlayLyricInfo 的 trans/roma 两轨(见 qq.go qqQRCLyric /
		...
		case "kugou":
			// 加。酷狗 KRC `[language:]` 内嵌的中文译文 / 罗马音(见 kugou.go
		...
		case "migu":
			// 加。咪咕 trcUrl 的译文固定中文,标 "zh",可用性由候选装配时的
...
func fetchScoredLyricCandidatesStreaming(ctx context.Context, artist, title, album string, durationSecs float64, onUpdate lyricSearchUpdateFunc) (neteaseInfo, []scoredLyricCandidateResult) {
	...
	go func() {
		...
		if qqMid != "" {
			...
			// 见 qq.go 顶部注释。同一份响应还带中文译文/罗马音两轨,时间戳与上面 qqLyric 的整行歌词逐行一致。
		...
		// qqCover:加。qqSongCoverAndSinger 早就写好、一直只被 qqCoverFallback
	...
	go func() {
		...
		// 监听 ctx.Done(),一旦本轮被主动取消即刻退出,不悬空等待。
	...
	// 最后回)。接入第十个源时发现:那一刻 11 个 goroutine 只收 9 份,新接的
	// deezer 明明取回了 2810 字节逐行歌词、却从没进过候选列表。`git log -S` 查下来这个
	// 数字自引入起一次都没改过,也就是说 接酷我、09-04 接咪咕时就已经在丢一份
...
// 过不串行的两种未命中:①两个保存都用 pid 命名的同一个 tmp 文件,
