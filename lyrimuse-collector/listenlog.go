...
//
//  1. 没连 Last.fm(`p.lfm == nil`)—— 常规路径,见下面 那段收窄。
...
// 那是 收窄**之前**的行为。旧结论留在文件最显眼的位置、新结论埋在下面第 ~96
// 行,通盘梳理时验证这误导过判断,故改成开头就把当前条件讲清楚。
//
...
type listenLogLine struct {
	...
	//
	// 靠 dry-run 匹配到:少了这个字段,一个**已经连着** Last.fm 的用户,日志里每
...
// ⚠️ 调用方只有两种情况该调它:
//
//  1. **当前没有在往 Last.fm 提交**(p.lfm == nil)—— 常规路径,见下面 那段。
