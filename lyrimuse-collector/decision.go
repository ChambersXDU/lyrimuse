...
// 给出另一组候选(popup 显示 qq 482 赢,实际当时是 Musixmatch 962 赢,
...
type lyricsDecisionCandidate struct {
	...
	//
	// (search-lyrics 真查一次《Shall We Dance (Live)》):网易云/酷狗/QQ/
	...
	// ConsensusPeers:这条候选的正文跟**哪些**其它源高度一致(借鉴清单 V2,)。
...
// 那边 default 分支是"原样显示原始值",漏了就是界面上直接印一个英文串给用户看。// 加 manual-rematch 时就这么漏过一次(用户截图反馈「这里的文案是否没做好中文的」)。
