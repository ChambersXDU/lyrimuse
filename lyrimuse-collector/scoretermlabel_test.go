...
//
//	① 决策 path "manual-rematch",忘了补 Swift 译名,弹窗直接印英文串给用户看,
//	   用户截图问「这里的文案是否没做好中文的」→ 于是有了 lyricsdecisionpath_test.go;
//	② 打分项 "sourceDurationOff",**照样又漏了一次** —— score_terms 这条当时
...
//
// 所以这一类也固定:新增打分项 → 加进 lyricScoreTermKinds → 必须在那个 Swift switch 里
