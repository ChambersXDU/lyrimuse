...
func TestBuildYTMusicAdAppleScript(t *testing.T) {
	for _, family := range []string{"chromium", "safari"} {
		...
		// 超时秒数必须来自那个常量,不能是另写的字面量(这一条 真的写错过一次:
		// 常量之外又硬编码了一个 "4",两处会漂)。
...
func TestYTMusicAdProbeJSHasNoDoubleQuotes(t *testing.T) {
	...
	// 后两个是 的专辑名读取(Swift 侧 selftest 有同款守卫,两边同时改)。
...
// 第四段:页面上读到的专辑名。
//
// 起因是处理「YouTube Music 播一张专辑时,第一首歌不上送专辑名」——那是 YT Music
