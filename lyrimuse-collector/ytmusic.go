...
//
// 为什么要按 sourceMessage 过滤,不是"YouTube Music 查到什么就收什么":测试(见下面覆盖率
...
// 是逐字段读它的源码 + 自己发裸 HTTP 请求测试核实过一遍才落的这份实现,
...
// 逐音节证据(读 sigma67/ytmusicapi 的 LyricLine 模型 + 测试多首歌词证实,
...
const (
	...
	// 领域测试多年、被验证不会被拦的一个值,没有理由自己另编一个没验证过的。
	...
	// "AWoMEA4QChADEAQQCRAF")。⚠️ **不能不传这个参数**:验证,不带过滤器
	...
	// only",验证:同一个 browseId,WEB_REMIX 身份下 browse 只会返回
...
var (
	...
	// 一次",会同时打十几个请求到 YouTube 首页。这个坑 刚在 musixmatch 的
...
// get_visitor_id 同一个正则,核实过真的能从首页 HTML 里抠出来。
...
func ytmusicFetchVisitorID(ctx context.Context) string {
	...
	if v == "" {
		// 验证的一种具体失败原因:YouTube Music 按 IP 地理位置限定可用
...
// InnerTube 是没有文档的私有协议,这条路径 是这个形状,但 amll/musixmatch
...
func resolveYTMusicLyric(ctx context.Context, artist, title, album string, durationSecs float64) ytmusicResult {
	...
	// ⚠️ 用户追问验证:只在真是 LyricFind 时才接受这份候选,是 Musixmatch
	...
	//   ② 6/9 的测试命中就是这种"重复重发"(见 ytmusicLyric 文档注释里的覆盖率数据),
...
// 纯函数,便于单测;大小写不敏感只是防御性写法,测试两个取值大小写固定,没见过变体。
