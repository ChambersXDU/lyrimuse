...
//
// 为什么需要:lyrics_tr 目前只在网易云/Musixmatch 恰好带社区翻译时才有。测试本机缓存
// 179 条有歌词的记录里带译文的 40 条(22%),而"歌词里没有中文"的外语歌 32 条里带译文的
// 只有 1 条 —— 也就是听英文/日文歌时 97% 看不到翻译,恰恰是最需要翻译的场景。
//
// 为什么是 MyMemory:免费、不需要 API key(可选传邮箱提额),而且测试日译中/英译中的质量
// 对"看个大意"这个用途够用:
//
//	君のことが好きだから      → 因为我喜欢你
...
const (
	...
	// 警告文本(测试:"MYMEMORY WARNING: YOU USED ALL AVAILABLE FREE TRANSLATIONS FOR
...
func translationUsable(e enrichEntry, target string) bool {
	...
	//
	// 匹配到:App 侧"采纳候选/手改译文"那条路(EnrichCacheStore.saveEdit)
...
// ── 逐行按文字系统分流 ───────────────────────────────────────────────────────
//
// 验证的真 bug:宇多田ヒカル 的 First Love 是**日英混排**(主歌日文、
...
func translateChunk(ctx context.Context, hc *http.Client, baseURL string, lines []string, target string) ([]string, bool, error) {
	...
	// autodetect:测试跟显式指定源语言结果一致,省掉自己做语种识别这一整块。
...
// 译文语言从英文改回中文之后,以前听过的歌仍然显示英文译文,直到碰巧再放一次 —— // 处理就是这个。当时测试本机缓存:12 条语言不匹配,其中 11 条"会在下次播放时重翻",
// 也就是机制在、只是没有触发的机会。
//
...
func backfillTranslation(ctx context.Context, key string) {
	...
	defer func() {
		...
		// 中途才补出来的译文,要等下一次换歌才会被推出去(处理"为什么当前这
