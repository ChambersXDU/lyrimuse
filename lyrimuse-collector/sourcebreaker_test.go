...
// "每熔断一轮"是 正过的口径(原来是每失败一个请求升一档),所以这里每升一档
...
// 于是一次抖动就把阶梯走到头。测试日志:QQ 在 14:38:19.804 这同一毫秒里连跳 15s→30s→1m→2m
...
// 的 DNS 轨迹 —— 那是评审匹配到的坑:各源 client 都设了 Client.Timeout,DNS **挂住**时 Go 会把
