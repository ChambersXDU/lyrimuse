...
//
// ⚠️ 判据是**上一拍**在不在播,不是这一拍。按"这一拍在播"累加的话,
// 「上一拍暂停、这一拍恢复」会把整段暂停间隔当成播放时间加进去,停得越久跳得越多(App 侧暂停时轮询降到 6s、
// 空闲 10s;collector 是 5s)。处理「暂停久一点再恢复,歌词进度就不正常」,App 日志里的 resume transition
// 逐条验证跳了 3.5~5.8 秒。反过来「上一拍在播、这一拍暂停」照旧累加:那一段确实基本都在播。
