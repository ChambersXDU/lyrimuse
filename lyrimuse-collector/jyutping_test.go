...
// review 验证的真实 bug:jyut6ping3.words.dict.yaml 里混了 696 条带逗号/
...
func TestToJyutpingLine(t *testing.T) {
	cases := []struct {
		...
		// :"這個"在 JyutpingWords.txt 里单独收了 ze3 go3,跟"這"单字表的
		...
		{"空串", "", ""},
		// :词级多音字消歧(JyutpingWords.txt,同一个 rime-cantonese 项目)。
		...
		// :碰撞覆盖表(JyutpingCollisionOverrides.txt)。"离"这个简体字
		...
		// 的真 bug:拉丁/数字**紧接**汉字(中间没有空格)时,读音会被粘在
